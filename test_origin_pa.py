...
from utils import check_dir, set_determ, Recorder
...
def main():
    SAVE_PATH = os.path.join("./tmp/url", args['setting_name'], args['experiment_name']) # dir for experiment results analysis

    set_determ(args['seed'])
    TEST_SIZE = 600
    ...
    with tf.compat.v1.Session(config=config) as session:
        ...
        for dataset in testsets:
            ...
            for i in tqdm(range(TEST_SIZE)):
                ...
                selection_params, tmp_recorder = pa(context_features, context_labels, target_features, target_labels, max_iter=40, lr=lr, distance=args['test.distance'])
    ...
    res_recorder.save(filename=args['experiment_name'])
